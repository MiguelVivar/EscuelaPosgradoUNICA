    ...
    async buscarDocentes(filtros: FiltrosDocente = {}): Promise<{ docentes: Docente[] }> {
        try {
        ...
        const mockDocentes: Docente[] = [
            ...
            experienciaAnios: 15,
            sedes: [
                { id: 1, nombre: 'Chincha', direccion: 'Av. Universitaria 123', activa: true }
            ],
            facultades: [
            ...
            facultades: [
                { id: 2, nombre: 'Facultad de Ciencias Empresariales', codigo: 'FAC-EMP' }
            ],
    ...
    async updateDocente(id: number, data: Partial<DocenteForm>): Promise<Docente> {
        try {
        const response = await fetch(`${this.baseUrl}/${id}`, {
            method: 'PUT',
            headers: {
    ...
    async uploadExcel(file: File): Promise<{ procesados: number; errores: number; detalles: string[] }> {
        try {
