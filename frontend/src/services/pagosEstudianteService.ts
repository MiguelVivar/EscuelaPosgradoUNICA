...
class PagosEstudianteService {
  ...
  async getPagos(filtros?: {
    ...
    try {
      ...
      if (filtros?.estudianteId)
        params.append("estudianteId", filtros.estudianteId.toString());
      if (filtros?.estado) params.append("estado", filtros.estado);
      if (filtros?.tasaPagoId)
        params.append("tasaPagoId", filtros.tasaPagoId.toString());
  ...
  async createPagoEstudiante(
    data: PagoEstudianteForm
  ): Promise<PagoEstudiante> {
    try {
      // Simulación - reemplazar con llamada real a la API
  ...
  async registrarPago(data: RegistrarPagoForm): Promise<PagoEstudiante> {
    try {
      const response = await fetch(
        `${this.baseUrl}/${data.pagoEstudianteId}/registrar-pago`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async getEstadisticasPago(): Promise<EstadisticasPago> {
    try {
      // Simulación - reemplazar con llamada real a la API
      return {
        totalPendientes: 5,
        totalPagados: 15,
        totalVencidos: 2,
        montoTotalPendiente: 2500.0,
        montoTotalPagado: 7500.0,
