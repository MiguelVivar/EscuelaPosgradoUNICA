...
export default function MatriculaPage() {
  ...
  if (
    !isAuthenticated ||
    !user ||
    !["ADMIN", "COORDINADOR", "ALUMNO"].includes(user.role)
  ) {
    return null;
  }
      ...
      ? [
          {
            ...
            description:
              "Realice su matrícula en línea de manera fácil y rápida",
            icon: FiBookOpen,
          ...
          },
          {
            ...
            description:
              "Visualice reportes detallados y análisis de su rendimiento",
            icon: FiBarChart,
    ...
    ...(["ADMIN", "COORDINADOR"].includes(user.role)
      ? [
          ...
          },
          {
            title: "Facultades",
            description:
              "Administrar facultades para otorgar grados académicos",
            icon: FiBookOpen,
          ...
          },
          {
            ...
            description:
              "Visualización del rendimiento académico de los estudiantes",
            icon: FiCreditCard,
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-amber-50 py-8">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 mb-8">{welcome.subtitle}</p>
          <div className="flex justify-center">
            <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 shadow-lg border">
              <p className="text-gray-700">
                <span className="font-semibold">Usuario:</span> {user.nombres}{" "}
                {user.apellidos}
            ...
            {menuItems.slice(0, 3).map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className="group block">
                  <div
                    className={`${item.bgColor} border-2 ${item.borderColor} rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:scale-105 backdrop-blur-sm text-center`}
                  >
                    <div
                      className={`inline-flex p-4 rounded-xl bg-gradient-to-br ${item.color} text-white mb-6 group-hover:scale-110 transition-transform duration-300 mx-auto`}
                    >
                      <IconComponent size={32} />
                    </div>

                    <h3
                      className={`text-xl font-bold ${item.textColor} mb-3 group-hover:text-opacity-80`}
                    >
                      {item.title}
                    </h3>
        ...
        {user.role === "ALUMNO" && (
          <div className="mt-16 bg-white/80 backdrop-blur-sm rounded-2xl p-8 shadow-xl border">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Button
                onClick={() =>
                  router.push("/campus-virtual/matricula/seguimiento")
                }
