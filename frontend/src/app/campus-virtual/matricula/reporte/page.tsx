"use client";
        ...
        <div className="space-y-4">
            ...
            {data.labels.map((label: string, index: number) => {
                ...
                <div key={index} className="text-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-lg font-bold text-blue-600">{value.toLocaleString()}</div>
                </div>
                );
            })}
            </div>
    ...
    function ReporteMatriculasComponent({ reporte }: { reporte: ReporteMatriculas }) {
    const chartData = {
    ...
    return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {reporte.totalMatriculados > 0 ? (
                ...
                <h4 className="font-medium text-gray-900 mb-2">Matriculados por Programa</h4>
                <p className="text-gray-500">No hay estudiantes matriculados aún</p>
                </div>
            )}
            </div>
            ...
            <div className="text-center py-8">
                ...
                <h4 className="font-medium text-gray-900 mb-2">Evolución de Matrículas</h4>
                <p className="text-gray-500 text-sm mb-2">
                Muestra el crecimiento de matrículas a lo largo del período académico
                </p>
                <p className="text-gray-400 text-xs">
                Los datos se mostrarán cuando haya registros históricos de matrículas
                </p>
            </div>
            </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            {reporte.matriculadosPorPrograma.length === 0 && (
                <div className="text-center py-8">
                <p className="text-gray-500">No hay programas con estudiantes matriculados</p>
                </div>
            )}
            </div>
        </div>
    ...
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <StatCard
            title="% Cumplimiento"
            value={`${reporte.estadisticasPagos.porcentajeCumplimiento.toFixed(1)}%`}
            icon={<FaChartPie />}
            color="bg-blue-500"
            isText={true}
            />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {reporte.totalRecaudado > 0 ? (
                ...
                <h4 className="font-medium text-gray-900 mb-2">Recaudación por Tipo</h4>
            ...
            <div className="text-center py-8">
                ...
                <h4 className="font-medium text-gray-900 mb-2">Recaudación Mensual</h4>
                <p className="text-gray-500">Los datos se mostrarán cuando haya pagos registrados</p>
            </div>
            </div>
        </div>
        </div>
    ...
    return (
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="text-center py-8">
                <FaChartBar className="mx-auto text-4xl text-gray-300 mb-4" />
                <h4 className="font-medium text-gray-900 mb-2">Rendimiento por Programa</h4>
                <p className="text-gray-500">Los datos se mostrarán cuando haya calificaciones registradas</p>
            </div>
            )}
        </div>
        </div>
    ...
    const [estudianteExpandido, setEstudianteExpandido] = useState<number | null>(null);
    const [vistaDetalle, setVistaDetalle] = useState<'calificaciones' | 'asistencias' | 'pagos'>('calificaciones');
    ...
    const estudiantesFiltrados = reporte.estudiantes.filter(estudiante => {
        ...
        const matchPrograma = !filtroPrograma || estudiante.programa === filtroPrograma;
        const matchEstado = !filtroEstado || estudiante.estado === filtroEstado;
        
        return matchSearch && matchPrograma && matchEstado;
    });

    const programasUnicos = [...new Set(reporte.estudiantes.map(e => e.programa))];
    const estadosUnicos = [...new Set(reporte.estudiantes.map(e => e.estado))];

    const toggleEstudiante = (estudianteId: number) => {
        setEstudianteExpandido(estudianteExpandido === estudianteId ? null : estudianteId);
    };

    const getEstadoColor = (estado: string) => {
        switch (estado) {
        case 'ACTIVO': return 'bg-green-100 text-green-800';
        case 'INGRESANTE': return 'bg-blue-100 text-blue-800';
            ...
            {estudiantesFiltrados.map((estudiante) => (
                ...
                >
                    ...
                    <div>
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getEstadoColor(estudiante.estado)}`}>
                            {estudiante.estado}
                        </span>
                    ...
                    {vistaDetalle === 'calificaciones' && (
                        ...
                        {estudiante.calificaciones.length > 0 ? (
                            ...
                            <div key={index} className="bg-white p-3 rounded border">
                                ...
                                <div>
                                    <h5 className="font-medium text-gray-900">{calificacion.asignatura}</h5>
                                    <p className="text-sm text-gray-600">{calificacion.evaluacion}</p>
                                    <p className="text-xs text-gray-500">{calificacion.fecha}</p>
                                </div>
                                <div className="text-right">
                                    <span className={`text-lg font-bold ${getNotaColor(calificacion.nota)}`}>
                            ...
                            <p className="text-center text-gray-500 py-4">No hay calificaciones registradas</p>
                        )}
                        </div>
                    )}
                    ...
                    {vistaDetalle === 'asistencias' && (
                        ...
                        {estudiante.asistencias.length > 0 ? (
                            ...
                            <div key={index} className="bg-white p-3 rounded border">
                                ...
                                <div>
                                    <h5 className="font-medium text-gray-900">{asistencia.asignatura}</h5>
                                    <p className="text-sm text-gray-600">{asistencia.fecha}</p>
                                </div>
                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getAsistenciaColor(asistencia.estado)}`}>
                                    {asistencia.estado}
                                </span>
                                </div>
                            ...
                            <p className="text-center text-gray-500 py-4">No hay registros de asistencia</p>
                        )}
                        </div>
                    )}
                    ...
                    {vistaDetalle === 'pagos' && (
                        ...
                        {estudiante.pagos.length > 0 ? (
                            ...
                            <div key={index} className="bg-white p-3 rounded border">
                                <div className="flex justify-between items-start">
                                <div>
                                    <h5 className="font-medium text-gray-900">{pago.concepto}</h5>
                                    <p className="text-sm text-gray-600">
                                ...
                                <div className="text-right">
                                    ...
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getPagoColor(pago.estado)}`}>
                                    {pago.estado}
                                    </span>
                                </div>
                                </div>
    ...
    // Estados principales
    const [reporteMatriculas, setReporteMatriculas] = useState<ReporteMatriculas | null>(null);
    const [reportePagos, setReportePagos] = useState<ReportePagos | null>(null);
    const [reporteAcademico, setReporteAcademico] = useState<ReporteAcademico | null>(null);
    ...
    const [tipoReporteActivo, setTipoReporteActivo] = useState<TipoReporte>(TipoReporte.MATRICULAS);
    ...
    const loadReporte = async () => {
        ...
        switch (tipoReporteActivo) {
            ...
            const programasActivos = await programasEstudioService.getProgramasActivos();
            
            const matriculasReales: ReporteMatriculas = {
                periodoAcademico: {
                id: filtros.periodoAcademicoId || 1,
                nombre: periodosAcademicos.find(p => p.id === filtros.periodoAcademicoId)?.nombre || '2024-I',
                codigo: '2024-1',
                },
                totalMatriculados: estudiantes.length,
                matriculadosPorPrograma: programasActivos.map(programa => {
                const estudiantesPrograma = estudiantes.filter(e => e.programa?.id === programa.id);
                return {
                    programaId: programa.id,
                    programaNombre: programa.nombre,
                    facultad: programa.facultad?.nombre || 'Sin facultad',
                    totalMatriculados: estudiantesPrograma.length,
                    nuevosIngresos: estudiantesPrograma.filter(e => e.estado === 'INGRESANTE').length,
                    reingresantes: estudiantesPrograma.filter(e => e.estado === 'ACTIVO').length,
                    porcentaje: estudiantes.length > 0 ? (estudiantesPrograma.length / estudiantes.length) * 100 : 0
                };
                }),
                matriculadosPorSede: sedes.map(sede => ({
                sedeId: sede.id,
                sedeNombre: sede.nombre,
                totalMatriculados: estudiantes.filter(e => e.sede?.id === sede.id).length,
                porcentaje: estudiantes.length > 0 ? (estudiantes.filter(e => e.sede?.id === sede.id).length / estudiantes.length) * 100 : 0
                })),
                matriculadosPorEstado: [
                { estado: 'ACTIVO', cantidad: estudiantes.filter(e => e.estado === 'ACTIVO').length, porcentaje: estudiantes.length > 0 ? (estudiantes.filter(e => e.estado === 'ACTIVO').length / estudiantes.length) * 100 : 0 },
                { estado: 'INGRESANTE', cantidad: estudiantes.filter(e => e.estado === 'INGRESANTE').length, porcentaje: estudiantes.length > 0 ? (estudiantes.filter(e => e.estado === 'INGRESANTE').length / estudiantes.length) * 100 : 0 },
                { estado: 'GRADUADO', cantidad: estudiantes.filter(e => e.estado === 'GRADUADO').length, porcentaje: estudiantes.length > 0 ? (estudiantes.filter(e => e.estado === 'GRADUADO').length / estudiantes.length) * 100 : 0 },
                { estado: 'RETIRADO', cantidad: estudiantes.filter(e => e.estado === 'RETIRADO').length, porcentaje: estudiantes.length > 0 ? (estudiantes.filter(e => e.estado === 'RETIRADO').length / estudiantes.length) * 100 : 0 }
                ],
                evolucionMatriculas: [],
                ...
                porcentajeHombres: 0,
                porcentajeMujeres: 0,
                estudiantesRegulares: estudiantes.filter(e => e.estado === 'ACTIVO').length,
                estudiantesIrregulares: estudiantes.filter(e => e.estado !== 'ACTIVO' && e.estado !== 'GRADUADO').length,
                tasaRetencion: 0,
                tasaDesercion: 0
                }
            };
            ...
            try {
                ...
                totalRecaudado: 0,
                totalPendiente: 0,
                pagosPorTipo: tasasPago.length > 0 ? tasasPago.map(tasa => ({
            ...
            const estudiantesDetallados: EstudianteDetallado[] = estudiantesDetalle.map(estudiante => ({
                id: estudiante.id,
                ...
                fechaMatricula: estudiante.fechaRegistro ? new Date(estudiante.fechaRegistro).toLocaleDateString() : 'No disponible',
                calificaciones: [
                {
                ...
                },
                {
                    fecha: '2024-03-20',
                    asignatura: 'Metodología de la Investigación',
                    estado: Math.random() > 0.8 ? 'TARDANZA' as const : 'PRESENTE' as const,
                    observaciones: Math.random() > 0.8 ? 'Llegó 10 minutos tarde' : undefined
                }
                ],
                pagos: [
                {
                ...
                },
                {
                    ...
                    monto: 350.00,
                    estado: Math.random() > 0.7 ? 'PENDIENTE' as const : 'PAGADO' as const,
            ...
            const reporteDetalladoReal: ReporteDetallado = {
                ...
                estudiantesConCalificaciones: estudiantesDetallados.filter(e => e.calificaciones.length > 0).length,
                estudiantesConPagosAlDia: estudiantesDetallados.filter(e => 
                    e.pagos.every(p => p.estado === 'PAGADO')
                ).length,
                promedioAsistenciaGeneral: estudiantesDetallados.length > 0 ? 
                    (estudiantesDetallados.reduce((acc, e) => {
                    const asistenciasPresente = e.asistencias.filter(a => a.estado === 'PRESENTE').length;
                    return acc + (e.asistencias.length > 0 ? (asistenciasPresente / e.asistencias.length) * 100 : 0);
                    }, 0) / estudiantesDetallados.length) : 0
                }
            };
    ...
    const handleExportReport = async (formato: 'PDF' | 'EXCEL') => {
        ...
        const reporteData = {
            ...
            filtros,
            data: tipoReporteActivo === TipoReporte.MATRICULAS ? reporteMatriculas :
                tipoReporteActivo === TipoReporte.PAGOS ? reportePagos : reporteAcademico
        };

        // Crear un blob con los datos del reporte
        const content = JSON.stringify(reporteData, null, 2);
        const blob = new Blob([content], { type: formato === 'PDF' ? 'application/pdf' : 'application/vnd.ms-excel' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `reporte-${tipoReporteActivo.toLowerCase()}-${new Date().toISOString().split('T')[0]}.${formato.toLowerCase() === 'excel' ? 'json' : 'txt'}`;
    ...
    return (
        ...
        <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
            ...
            <div className="text-sm text-gray-600">
                Período: {periodosAcademicos.find(p => p.id === filtros.periodoAcademicoId)?.nombre}
            </div>
            )}
        </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
                ...
                <div>
                    ...
                    onChange={(e) =>
                        setTempFiltros({
                        ...tempFiltros,
                        periodoAcademicoId: e.target.value ? Number(e.target.value) : undefined,
                        })
                    }
                    >
