...
export default function InfoDocentePage() {
  ...
  const [docenteCompleto, setDocenteCompleto] = useState<Docente | null>(null);
  const [editFormData, setEditFormData] = useState<DocenteForm | null>(null);
  const [selectedDocentePagos, setSelectedDocentePagos] = useState<
    PagoDocente[]
  >([]);
  ...
  const handleEditDocente = async (docente: Docente) => {
    ...
    setEditFormData({
      ...
      sedesIds: docente.sedes.map((s) => s.id),
      facultadesIds: docente.facultades?.map((f) => f.id) || [],
    });
  ...
  const handleSaveEdit = async () => {
    if (!selectedDocente || !editFormData) return;

    try {
      const sedesSeleccionadas = sedes.filter((s) =>
        selectedSedes.includes(s.id)
      );
  ...
  const filtered = useMemo(() => {
    return docentes.filter((d) => {
      ...
      if (
        filtros.sedeId &&
        !d.sedes?.some((s) => s.id.toString() === filtros.sedeId)
      ) {
        return false;
      }

      if (
        filtros.experienciaMinima &&
        d.experienciaAnios < filtros.experienciaMinima
      ) {
        return false;
      }
      ...
      if (
        filtros.tipoGrado &&
        !d.grados?.some((g) => g.tipo === filtros.tipoGrado)
      ) {
        return false;
      }
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end space-x-2">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleViewAcademicInfo(d)}
                        leftIcon={FaBook}
                      >
                        Académico
                      </Button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    type="text"
                    value={editFormData?.especialidades?.join(", ") || ""}
                    onChange={(e) =>
                      editFormData &&
                      setEditFormData({
                        ...editFormData,
                    ...
                    <div className="bg-blue-50 p-6 rounded-lg border border-blue-200 text-center">
                      ...
                      <p className="text-3xl font-bold text-blue-800">
                        ...
                        {selectedDocentePagos.length > 0
                          ? (
                              selectedDocentePagos.reduce(
                                (sum, p) => sum + p.sueldoNeto,
                                0
                              ) / selectedDocentePagos.length
                            ).toFixed(0)
                          : "0"}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm">
                          ...
                          {docenteCompleto.experienciaAnios} años
                        </p>
                        <p className="text-sm">
                          <strong>Estado:</strong>{" "}
                          <span
                            className={`ml-2 px-2 py-1 rounded-full text-xs font-semibold ${getEstadoBadge(
                              docenteCompleto.estado
                            )}`}
                          >
                            {
                              ESTADOS_DOCENTE_LABELS[
                                docenteCompleto.estado as EstadoDocente
                              ]
                            }
                          </span>
                        </p>
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {docenteCompleto.sedes?.map((sede) => (
                          <span
                            key={sede.id}
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm">
                      ...
                      <div className="space-y-2">
                        {docenteCompleto.grados?.length ? (
                          docenteCompleto.grados.map((grado) => (
                            <div key={grado.id} className="text-sm">
                              <strong>
                                {TIPOS_GRADO_LABELS[grado.tipo as TipoGrado]}
                              </strong>{" "}
                              - {grado.institucion} ({grado.anioObtencion})
                            </div>
                          ))
                        ) : (
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm">
                          ...
                          {docenteCompleto.especialidades?.join(", ") || "N/A"}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTempFiltros({
                      ...tempFiltros,
                      experienciaMinima: parseInt(e.target.value),
                    })
                  }
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                />
            ...
            <div className="p-6">
              ...
              <div className="mb-6 p-4 bg-amber-50 rounded-lg border border-amber-200">
                ...
                <ul className="text-sm text-amber-700 space-y-1">
                  ...
                  <li>
                    • Los códigos de docente deben ser <strong>únicos</strong>
                  </li>
                </ul>
                <button
