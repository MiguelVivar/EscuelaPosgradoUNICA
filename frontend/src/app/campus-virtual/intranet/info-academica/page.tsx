...
export default function InfoAcademicaPage() {
  ...
  const [informacionAcademica, setInformacionAcademica] = useState<InformacionAcademica | null>(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'resumen' | 'historial' | 'proyeccion'>('resumen');
  ...
  useEffect(() => {
    const cargarInformacionAcademica = async () => {
      try {
        ...
        const data = await academicService.getInformacionAcademica(estudianteId);
  ...
  const getEstadoMateria = (estado: string) => {
    const estados = {
      ...
      'REPROBADO': { color: 'bg-red-100 text-red-800', icon: FaExclamationTriangle },
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-lg">No se encontró información académica</p>
        </div>
      </div>
    );
  }

  const { estudiante, historialAcademico, resumenAcademico, proyeccion } = informacionAcademica;
        ...
        <div className="border-b border-gray-200">
          <nav className="-mb-px flex space-x-8">
            {[
              ...
              { id: 'historial', label: 'Historial Académico', icon: FaBookOpen },
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg border-l-4 border-l-green-500">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">{resumenAcademico.promedioAcumulado.toFixed(2)}</p>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg border-l-4 border-l-blue-500">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">{resumenAcademico.porcentajeAvance.toFixed(1)}%</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">
                    {resumenAcademico.creditosTotalesAprobados}/{resumenAcademico.creditosTotalesRequeridos}
                  </p>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg border-l-4 border-l-purple-500">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">{estudiante.semestreActual}°</p>
                </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-sm text-gray-600">{resumenAcademico.porcentajeAvance.toFixed(1)}%</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-semibold">{resumenAcademico.materiasAprobadas}</span>
                </div>
                <div>
                  <span className="text-gray-600">Materias Reprobadas:</span>
                  <span className="ml-2 font-semibold">{resumenAcademico.materiasReprobadas}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'historial' && (
        <div className="space-y-6">
          {historialAcademico.periodos.map((periodo) => (
            <div key={periodo.id} className="bg-white rounded-lg shadow-lg overflow-hidden">
              <div className="bg-gray-50 p-4 border-b">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-lg font-semibold">{periodo.periodo.nombre}</h3>
                    <p className="text-sm text-gray-600">{periodo.periodo.codigo}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-600">Promedio del Semestre</p>
                    <p className="text-2xl font-bold text-amber-600">{periodo.promedioSemestre.toFixed(2)}</p>
                  </div>
                </div>
              </div>
                    ...
                    {periodo.materias.map((materia) => {
                      ...
                      return (
                        <tr key={materia.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-900">{materia.creditos}</td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getNotaBadge(materia.nota)}`}>
                              {materia.nota ? materia.nota.toFixed(1) : 'N/A'}
                            </span>
                          </td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full ${estadoConfig.color}`}>
                              <IconEstado className="mr-1" size={12} />
                              {materia.estado}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-900">{materia.profesor}</td>
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-lg shadow-lg">
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-sm text-gray-600">Fecha Estimada de Graduación</p>
                  <p className="text-xl font-semibold text-amber-600">
                    {new Date(proyeccion.fechaEstimadaGraduacion).toLocaleDateString()}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Semestres Pendientes</p>
                  <p className="text-xl font-semibold">{proyeccion.semestresPendientes}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Créditos Pendientes</p>
                  <p className="text-xl font-semibold">{proyeccion.creditosPendientes}</p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg">
            ...
            <div className="space-y-3">
              {proyeccion.recomendaciones.map((recomendacion, index) => (
                <div key={index} className="flex items-start p-3 bg-amber-50 rounded-lg border-l-4 border-amber-500">
                  <FaInfoCircle className="mr-3 text-amber-500 mt-0.5 flex-shrink-0" size={16} />
